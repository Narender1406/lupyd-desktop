...
function TaskCard({
  ...
  return (
    <div className="rounded-lg border p-3 sm:p-4 bg-white dark:bg-[#0a0a0a] dark:border-[#222]">
      ...
      <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
        <div className="space-y-1">
          ...
          <Select value={task.status} onValueChange={(v: TaskStatus) => onChange({ ...task, status: v })}>
            <SelectTrigger className="h-9 dark:bg-[#0d0d0d] dark:border-[#222]">
              <SelectValue placeholder="Status" />
            </SelectTrigger>
            <SelectContent className="dark:bg-black dark:border-[#222]">
        ...
        <div className="space-y-1">
          ...
          <Input
            type="date"
            className="h-9 dark:bg-[#0d0d0d] dark:border-[#222]"
            value={task.due ?? ""}
...
function Column({ title, tasks, members, onUpdate }: any) {
  return (
    <div className="space-y-2">
      ...
      <div className="space-y-2">
        ...
        {tasks.length === 0 && (
          <div className="text-xs text-muted-foreground border rounded-lg p-3 bg-white dark:bg-[#0a0a0a] dark:border-[#222]">
            No tasks here.
          </div>
        )}
      </div>
...
export function GroupTasks({ open, onOpenChange, groupName, members }: any) {
  ...
  const [tasks, setTasks] = useState<Task[]>([
    ...
    { id: newId(), title: "Implement auth context", status: "inprogress", assigneeId: members[1]?.id, priority: "high" },
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="w-full sm:max-w-[560px] p-0 flex flex-col bg-white dark:bg-black dark:text-white"
      >
        <SheetHeader className="p-4 border-b dark:border-[#222]">
        ...
        {/* Controls */}
        <div className="p-3 sm:p-4 border-b space-y-3 dark:border-[#222]">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <div className="sm:col-span-2">
              ...
              <Input
                ...
                className="bg-gray-100 dark:bg-[#0d0d0d] dark:border-[#222] border-none"
              />
            </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          <Tabs defaultValue="todo" className="flex h-full flex-col">
            <div className="px-3 sm:px-4 py-2 border-b dark:border-[#222]">
              <TabsList className="bg-transparent p-0 h-auto gap-2 overflow-x-auto">
                ...
                <TabsTrigger
                  ...
                  className="rounded-none px-2 py-1
                    data-[state=active]:border-b-2 
                    data-[state=active]:border-black 
                    dark:data-[state=active]:border-white whitespace-nowrap"
                >
                  All ({counts.all})
                </TabsTrigger>
              </TabsList>
            ...
            <TabsContent value="all" className="flex-1 overflow-y-auto p-3 sm:p-4">
              <div className="space-y-6">
                <Column title="To do" tasks={by.todo} members={members} onUpdate={updateTask} />
                <Separator className="dark:bg-[#222]" />
                <Column title="In progress" tasks={by.inprogress} members={members} onUpdate={updateTask} />
                <Separator className="dark:bg-[#222]" />
                <Column title="Done" tasks={by.done} members={members} onUpdate={updateTask} />
              </div>
